...
### When to use the logging stack?
Currently Kubernetes uses the underlying container runtime logging which does not persist logs for not existing containers. This makes it difficult to investigate issues occurred in long ago destroyed containers.
...
After that there are two options for monitoring logs. The URLs can be found in the Gardener Dashboard in `Monitoring and Logging` section.
* The first one is to log in in the admin panel (usually located at the bottom left corner). The default username and password are `admin`, `admin` (After successful log in, you will be asked for changing the default password. It should not be changed because this dashboard will be used from all of the operators!). After that a new `Explore` menu will be available (left side of the screen). It is used for creating a log queries using the predefined filters in Loki. For example: 
